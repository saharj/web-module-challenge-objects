...
createMenuItem("Pizza", 16, "Lunch");
createMenuItem("Pancake", 13, "Breakfast");
...
function addNewReview(arr, name, rating, feedback) {
  arr.push({ name, rating, feedback });
  return arr;
}

addNewReview(reviews, "Sahar", 5, "My favorite hangout spot!");
console.log(reviews);
...
function getLastReview(arr) {
  ...
  return `${arr[lastIndx].name} gave the restaurant a ${arr[lastIndx].rating}, and their feedback was: ${arr[lastIndx].feedback}`;
}
...
function getLongReviews(arr) {
  ...
  let wordsSplit;
  for (let i = 0; i < arr.length; i++) {
    wordsSplit = arr[i].feedback.split(" ");

    if (wordsSplit.length > 15) {
      arrayToReturn.push(arr[i]);
    }
  }
